...
var window: UIWindow?
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        let landingViewController = LandingViewController(viewModel: LandingViewModel(loadDataSubject: ReplaySubject.create(bufferSize: 1), inspiringPeopleRelay: BehaviorRelay.init(value: []), inspiringPeopleRepository: InspiringPeopleRepositoryImpl(inspiringPeople: inspiringPeople), userInteractionSubject: PublishSubject(), showQuoteSubject: PublishSubject()))
        let addPersonViewController = AddPersonViewController(viewModel: AddPersonViewModel(inspiringPerson: InspiringPerson(image: nil, description: nil, birth: nil, death: nil, quotes: nil), userInteractionSubject: PublishSubject(), showAlertSubject: PublishSubject(), inspiringPersonRepository: landingViewController.viewModel.inspiringPeopleRepository, type: .add))
